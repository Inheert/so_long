...
void	create_player_collision(t_player *player)
{
	...
	if (!mlx_resize_image(collision->img,
			MLX_WIN_HEIGHT / HITBOX_SIZE_DIV, MLX_WIN_HEIGHT / HITBOX_SIZE_DIV)
...
void	initialize_player_components(mlx_t *mlx, t_player *player)
{
	...
	player->walking_sprites = create_animation_chain(
			player, true, S_HANDGUN_WALK_PATH, S_HANDGUN_WALK_COUNT, WALK);
	player->shoot_sprites = create_animation_chain(
			player, false, S_HANDGUN_SHOOT_PATH, S_HANDGUN_SHOOT_COUNT, SHOOT);
